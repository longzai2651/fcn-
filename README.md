...
|文件位置|描述
|-------|---
|linux\ `fcn32` | FCN x86平台bin
|linux\ `fcn` | FCN x64平台bin
|linux\ `fcn-arm` | FCN arm平台bin [PI3测试通过]
|linux\ `fcn-armbian` | FCN armbian平台bin [Orange Pi测试通过]
|linux\ `fcn-openwrt` | FCN openwrt mips平台bin [TPLINK WR703N测试通过]
|linux\ `fcn.conf` | FCN服务端配置文件模板
